"""
modules/core/settings/services/env_service.py
Updated: April 5, 2025
Environment variable handling with TTL-based caching for settings
"""
...
COMPONENT_ID = f"{MODULE_ID}.env_service"
...
class EnvironmentService:
    ...
    def __init__(self, ttl: int = 300):
        ...
        self.logger.debug(f"Initialized environment service with TTL of {ttl} seconds")
    ...
    async def initialize(self, app_context=None, settings=None) -> bool:
        ...
        """
        # Nothing to initialize - service is always ready
        return self.initialized
    
    async def get_env_var(self, env_var: str, default: Any = None) -> Result:
        """
        ...
        Returns:
            Result with environment variable value or default if not found
        """
        try:
            now = time.time()
            ...
            if env_var in self.cache:
                ...
                if now - cache_entry["timestamp"] < self.ttl:
                    return Result.success(data=cache_entry["value"])
                else:
            ...
            self.logger.error(f"Error getting environment variable {env_var}: {str(e)}")
            return Result.error(
                code="ENV_VAR_ERROR",
                message=f"Error getting environment variable {env_var}",
                details={"env_var": env_var, "error": str(e)}
            )
    
    async def get_env_vars_by_prefix(self, prefix: str, strip_prefix: bool = False) -> Result:
        """
        ...
        async def _get_env_vars():
            ...
            for env_var in os.environ:
                if env_var.startswith(prefix):
                    ...
                    value_result = await self.get_env_var(env_var)
        ...
        return await handle_result_operation(
            _get_env_vars,
            COMPONENT_ID,
            "ENV_VARS_BY_PREFIX_ERROR",
            f"Error getting environment variables with prefix {prefix}",
        ...
        try:
            ...
            if setting_keys is None:
                ...
                for key in setting_keys:
                    ...
                    value_result = await self.get_env_var(env_var)
                    if value_result.success and value_result.data is not None:
                        result[key] = value_result.data
                return Result.success(data=result)
        except Exception as e:
            self.logger.error(f"Error getting environment variables for module {module_id}: {str(e)}")
            return Result.error(
    ...
    async def clear_cache(self) -> Result:
        ...
        try:
            ...
            self.logger.error(f"Error clearing environment variable cache: {str(e)}")
            return Result.error(
    ...
    async def set_ttl(self, ttl: int) -> Result:
        ...
        try:
            ...
            self.logger.error(f"Error updating cache TTL: {str(e)}")
            return Result.error(
                code="SET_TTL_ERROR",
    ...
    async def refresh(self, env_var: str) -> Result:
        ...
        try:
            ...
            self.logger.error(f"Error refreshing environment variable {env_var}: {str(e)}")
            return Result.error(
                code="REFRESH_ERROR",
